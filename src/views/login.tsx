...
import { browserLocalPersistence, setPersistence, signInWithEmailAndPassword, updateProfile } from "firebase/auth";
...
const LoginView: React.FC = () => {
  ...
  const onSubmitSigninForm = async (values: any) => {
    setLoading(true);
    setPersistence(auth, browserLocalPersistence)
      .then(async () => {
        const userCredential = await signInWithEmailAndPassword(auth, values.email, values.password);
        setCookie("accessToken", userCredential?.user?.accessToken, { path: '/' });
        router.push("/users")
        setLoading(false);
      })
      .catch((error) => {
          ...
          <div className="w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2">
            <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
              <Formik
                ...
                onSubmit={(values) => onSubmitSigninForm(values)}
              >
